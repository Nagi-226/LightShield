...
> **背景**：单一 Agent 无法覆盖"BUG 排查 / 屎山冗余 / 耦合分析 / 可行性边界 / 优化修复"全部 5 个维度。按模型优势组合审查。

```
Phase 1: 发现（并行 — Kimi + ZCode）
┌──────────────────────────────────────────────────────┐
│  Kimi (K2.7-code)            ZCode (GLM-5.2)         │
│  🔬 深度 BUG 排查              🗑️🔗 屎山 + 耦合分析    │
│                                                        │
│  • 逐文件执行路径追踪          • 1M 上下文一次加载全项目│
│  • 状态变量生命周期分析        • 循环依赖 / 分层违规    │
│  • 异常路径覆盖检查            • 死代码 / 重复逻辑      │
│  • 唯一不同模型 → 发现同源盲区 • 改动影响面全局映射     │
└──────────────────┬──────────────┬─────────────────────┘
                   │              │
                   ▼              ▼
Phase 2: 验证（集中 — Codex）
...
│  • CC 判定哪些要修 / 哪些已知悉不修                    │
│  • 简单修复 → CC 直接改                               │
│  • 复杂修复 → QoderWork VM 隔离执行 + 真机验证         │
│  • 全量回归 771 tests 不下降                          │
└──────────────────────────────────────────────────────┘
```

**触发时机**：每里程碑版本封版前执行一轮（≈ 每 5-10 个小版本），或在发现多处 bug/耦合问题时提前触发。

### 🔑 ZCode 架构二审（v0.0.40+ 生效）

> **命题**：CC 的架构决策如果出错——分层语义偏差、ADR 与实现矛盾、抽象层级错误——其他 Agent 基于错误架构产出 = 项目跑偏。
...
**审什么**（不是审代码——那是 Codex 的活）：
- 分层语义是否自洽（如 `sandbox/` 的语义定义与实际使用是否一致）
- ADR 决策 vs 实现是否吻合（是否存在"ADR 说 A 但代码做 B"的矛盾）
- 跨模块接口契约是否一致（同一个概念在不同模块中是否有不同实现）
- 抽象层级是否合理（模板方法 vs 策略模式的选择是否会在后续版本变成枷锁）
- 遗漏关注点（如"rollback 失败怎么办"是否被定义）

**触发频率**：每里程碑版本一次（≈ 每 5-10 个小版本），不是每次 commit。CC 完成架构决策和实现后 → ZCode 做二审 → 发现问题 → ADR 修正 → 重新合入。
...
| **当前阶段** | v0.0.38 — 阶段三 1/2（沙箱执行器✅），664 tests / ruff+mypy 零违规 |
| **开源协议** | MIT |
| **主语言** | Python 3.10+ |
...
| v0.0.20 | CLI 发布 | Flask Web 面板 + 在线报告预览 |
| v0.0.30 | Web 面板 | 仪表板 + 加固建议 + 主题切换 + i18n |
| v0.0.40 | 自动加固 | 加固闭环（扫描→推荐→生成→执行→复扫→验证） |
| v0.0.50 | 质量基线 | 覆盖率 85% + 全部债务清零（0C/0H/0M/0L/0I） |
| v0.0.60 | RC 候选 | HTML 报告 + 规则增强 + WSGI 生产化 + 资产清单 + 全集群审计 → v1.0.0-rc1 |
...
| 扩展点 | 当前轻盾做法 (v0.0.49) | 未来重盾方向 | 切换机制 | 预留状态 |
|--------|------------------------|-------------|:--:|:--:|
