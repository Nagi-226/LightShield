...
| **当前阶段** | v0.0.49 — 质量基线已建立（1001 tests / 82.95% 覆盖率），v0.0.50–v0.0.60 迭代规划中 |
| **开源协议** | MIT |
| **主语言** | Python 3.10+ |
| **辅助语言** | Shell（部署脚本）、PowerShell（Windows 适配） |
| **最新更新** | 2026-07-08 — 进度追踪见 `.guardrails/PROGRESS.md` |
...
## 六、MVP 功能范围（v0.0.10）✅ 已完成

> 原则：做少做精，不做大而全。**v0.0.10 已全部交付，共 14 个 Python 模块。**
...
| P2 | Web 面板 | Flask 轻量管理界面 | v0.0.20 |
| P2 | 桌面客户端 | Tkinter 跨平台 GUI | v0.0.30 |
...
| **用户界面** | CLI（v0.0.10） | Web 面板 → 桌面客户端 → SaaS 多租户 | 前后端分离架构预留 |
...
| 版本 | 里程碑 | 核心交付 | 状态 |
|------|--------|---------|:--:|
| v0.0.10 | MVP | 资产扫描 + 漏洞检测 + 中文报告（CLI） | ✅ 已完成 |
| v0.0.20 | 完整 CLI 工具 | CLI 入口 + 自动加固 + 测试覆盖 + 开源文档 | 🟢 开发中 |
| v0.0.30 | Web 面板 | Flask 管理界面 + 在线报告预览 | ⏳ |
| v0.0.40 | 桌面客户端 | Tkinter 跨平台 GUI + 一键打包 | ⏳ |
